...
# Function to generate timetable PDF for a division
def generate_pdf_for_division(division_name, slots, output_dir, university_name="University Name"):
    # Ensure output directory exists
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    ...
    # Extract unique weekdays and slot timings
    # Weekdays in proper order (Monday to Friday)
    weekdays_order = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]
    weekdays = sorted(
        list({slot["working_day"]["day"]["name"].title() for slot in slots}),
        key=lambda x: weekdays_order.index(x),
    )

    slot_timings = sorted(
        list({(slot["slot_time"]["start_time"], slot["slot_time"]["end_time"]) for slot in slots})
    )
    ...
    day_to_col = {day: idx + 1 for idx, day in enumerate(weekdays)}  # Map day to column index
    for slot in slots:
        day = slot["working_day"]["day"]["name"].title()  # Convert to title case
        start_time = slot["slot_time"]["start_time"]
        end_time = slot["slot_time"]["end_time"]
        timing = (start_time, end_time)
        row_idx = timing_to_row[timing]
    ...
    # Add university name and timetable title
    styles = getSampleStyleSheet()
    title = Paragraph(f"<b>{university_name}</b>", styles["Title"])
    subtitle = Paragraph(f"<b>Timetable for Division: {division_name}</b>", styles["Heading2"])

    # Build PDF
    elements = [title, Spacer(1, 12), subtitle, Spacer(1, 24), table]
    pdf.build(elements)
    print(f"Timetable generated: {filename}")
